       ...
       nP = nNodes/2.0;
        ...
        for GM = numRange
            for scaleFac = scaleRange
                ...
                fprintf(fid,'\n#BSUB -R "rusage[mem=500]"   #Request RAM per process (CPU) for the job');
                fprintf(fid,'\n#BSUB -M 500                 #Set the per process enforceable memory limit.');
                ...
                fprintf(fid,'\nml myEB');
                fprintf(fid,'\nml OpenSees/3.2.0-foss-2020a-Python-3.8.2-parallel');
                fprintf(fid,'\ncd "$SCRATCH/Models/RunFiles"');
                fprintf(fid,'\nmpiexec -np %u OpenSeesSP <  RF_%0.0fyr_%0.0f%s%s%s.tcl',nNodes,t,frameID,scaledStr,nameSuffix,fmt);
                fprintf(fid,'\ncd "$SCRATCH/HPRC Frames/Output%s/Corr2_%0.0f/%u/Dynamic"',nameSuffix,frameID,t);
